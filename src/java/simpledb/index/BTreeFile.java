...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid,
		...
		while (iterator.hasNext()) {
			BTreeEntry nodeEntry = iterator.next();
			lastEntry = nodeEntry;
			if (f == null || f.compare(Op.LESS_THAN_OR_EQ, nodeEntry.getKey())) {
				return findLeafPage(tid, dirtypages, nodeEntry.getLeftChild(), perm, f);
			}
		}
		if (lastEntry == null) {
			throw new DbException("Internal tree was empty");
		}
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// split leaf page (add new page on the right of existing page)
		BTreeLeafPage right = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		BTreePageId rightPid = right.getId();

		// move half of the tuples to new page
		int n = page.getNumTuples();
		int toMove = n / 2; // moves the larger half
		Iterator<Tuple> it = page.reverseIterator();
		for (int i = 0; i < toMove && it.hasNext(); i++) {
			Tuple t = it.next();
			page.deleteTuple(t);
			right.insertTuple(t); // maintains the sorted order in the leaf
		}

		// update sibling pointers of affected leaf pages
		BTreePageId oldRightPid = page.getRightSiblingId();
		right.setRightSiblingId(oldRightPid);
		right.setLeftSiblingId(page.getId());
		page.setRightSiblingId(rightPid);

		if (oldRightPid != null) {
			BTreeLeafPage oldRight = (BTreeLeafPage) getPage(tid, dirtypages, oldRightPid, Permissions.READ_WRITE);
			oldRight.setLeftSiblingId(rightPid);
		}

		// copy middle key up into parent page
		Field splitKey = right.iterator().next().getField(keyField);
		...
		parent.insertEntry(new BTreeEntry(splitKey, page.getId(), rightPid));

		// return correct page
		if (field == null) {
			return page;
		}
		if (field.compare(Op.LESS_THAN, splitKey) || field.compare(Op.EQUALS, splitKey)) {
			return page;
		} else {
			return right;
		}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// we keep track of all entries first in an array
		...
		int n = entries.size();
		if (n == 0) { // added for error handling, in case that's why it fails test...
			throw new DbException("Trying to split an internal page with no entries");
		}

		// decide on the middle index
		int midIndex = n / 2;
		BTreeEntry middle = entries.get(midIndex);
		Field middleKey = middle.getKey();

		// split internal page (add new page on right of existing page)
		BTreeInternalPage right = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		BTreePageId rightPid = right.getId();

		// move half of entries to the new page
		...
		// insert left entries into 'page'
		for (int i = 0; i < midIndex; i++) {
			page.insertEntry(entries.get(i));
		}

		// insert right entries into 'right'
		for (int i = midIndex + 1; i < n; i++) {
			right.insertEntry(entries.get(i));
		}

		// update parent pointers of all children
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleKey); // middle key is pushed up to parent
		parent.insertEntry(new BTreeEntry(middleKey, page.getId(), rightPid)); // point to left & right page
		page.setParentId(parent.getId());
		right.setParentId(parent.getId());
		updateParentPointers(tid, dirtypages, right);

		// return page into which an entry with given key field should be inserted
		if (field == null) {
			return page;
		}
		if (field.compare(Op.LESS_THAN, middleKey) || field.compare(Op.EQUALS, middleKey)) {
			return page;
		} else {
			return right;
		}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if (moveTuple==null || !moveTuple.hasNext()) {
			throw new DbException("No tupes in sibiling");
		}
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> iter_l = leftSibling.reverseIterator();
		int numSteal = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;

		// Move parent's entry to page
		BTreeEntry entry = iter_l.next();
		BTreeEntry center = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(center);

		// Take out entry and insert it into the page
		for (int i=0; i < numSteal - 1; ++i) {
			if (!iter_l.hasNext()) {
				throw new DbException("No more entries in sibiling");
			}
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		Iterator<BTreeEntry> iter_r = rightSibling.iterator();
		int numSteal = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;

		// Move parent's entry to page
		BTreeEntry entry = iter_r.next();
		BTreeEntry center = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), entry.getLeftChild());
		page.insertEntry(center);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		Iterator<Tuple> iter_tuples = rightPage.iterator();
		if (!iter_tuples.hasNext()) {
			throw new DbException("no tuples in right page");
		}
		int numTuple = rightPage.getNumTuples();
		for (int i=0; i < numTuple; ++i) {
			Tuple tuple = iter_tuples.next();
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if (!iterator.hasNext()) {
			throw new DbException("no enrites in right page");
		}
		// insert the index node in parent into the left page
		BTreeEntry b_entry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(),
				rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(b_entry);
		while (iterator.hasNext()) {
		...
		// Update the parent page inserted into the child page
